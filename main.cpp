...
struct DynamicGrid {
    ...
    void ensure_capacity(int min_x, int min_y, int max_x, int max_y) {
        while (min_x - 2 < origin_x || max_x + 2 >= origin_x + width ||
            ...
            if (width >= 16384) break; // Hard limit to avoid infinite memory ballooning
            expand();
        }
    }
...
void handle_sigint(int sig) {
    if (in_render_phase) {
        std::cout << "\n\n[Interrupt] Ctrl+C caught during render! Exiting immediately.\n";
        std::exit(0);
    } else {
        std::cout << "\n\n[Interrupt] Ctrl+C caught! Stopping search and moving to render phase...\n";
...
// High-performance hash function to compute initial pipe types
...
int run_simulation(uint32_t seed, DynamicGrid &grid, std::vector<Point> &current_wave,
...
void save_bmp(const std::string &filename, int W, int H, const std::vector<Point> &wave, int min_x, int min_y) {
  static std::vector<uint8_t> full_buffer;
  int row_stride = (W * 3 + 3) & ~3;
  size_t total_size = (size_t)row_stride * H;
  
  if (full_buffer.size() < total_size) full_buffer.resize(total_size);
...
void search_worker() {
    ...
    w1.reserve(4096); w2.reserve(4096);

    while (keep_searching) {
        uint64_t next_seed_64 = global_seed_counter.fetch_add(1, std::memory_order_relaxed);
...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
      try {
          ...
          std::cout << "Target seed provided: " << best_seed << ". Skipping search phase.\n";
          
          DynamicGrid grid;
          std::vector<Point> w1, w2;
          int frames = run_simulation(best_seed, grid, w1, w2, nullptr, NoOpCallback{});
      ...
      std::cout << "Starting optimized search on " << num_threads << " threads...\n";

      std::vector<std::thread> threads;
      for (int i = 0; i < num_threads; ++i) threads.emplace_back(search_worker);
      ...
      while (keep_searching) {
        ...
        std::cout << "\rSearching... Seed: " << current 
                  ...
                  << std::setw(2) << std::setfill('0') << seconds << "s     " << std::flush;
  ...
  if (final_max > 0) {
    ...
    std::cout << "\n\nReplaying best seed: " << best_seed << " (" << final_max << " frames)\n";
    ...
    run_simulation(best_seed, grid, w1, w2, nullptr, [&](int frame, const std::vector<Point>& wave) {
        char fname[64];
        std::snprintf(fname, sizeof(fname), "bmps/state_%04d.bmp", frame);
        save_bmp(fname, W, H, wave, bound.x1, bound.y1);
        if (frame % 100 == 0) std::cout << "\rRendering... " << frame << "/" << final_max << std::flush;
    });
